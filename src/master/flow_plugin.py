...
def store_flows(client, message, expect_conf_id, now):
	...
	with database.transaction() as t:
		t.executemany("INSERT INTO flows (client, ip_from, ip_to, ip_remote, port_from, port_to, port_remote, proto, start, stop, opposite_start, size, count, inbound, seen_start) SELECT clients.id, %s, %s, %s, %s, %s, %s, %s, %s - %s * INTERVAL '1 millisecond', %s - %s * INTERVAL '1 millisecond', %s - %s * INTERVAL '1 millisecond', %s, %s, %s, %s FROM clients WHERE clients.name = %s", values)
