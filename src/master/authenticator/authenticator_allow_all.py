...
class AuthClient(basic.LineReceiver):
	...
	def lineReceived(self, line):
		...
		if match:
			...
			#with database.transaction() as t: #we add the users repeatedly.
			c=db.cursor()
			try:
				c.execute("INSERT INTO clients (name,passwd) VALUES ('%s','None')",str(client))
			except:
				db.rollback()
			else:
				db.commit()

			log_info = str(cred_cache.get(client.lower()))
			if log_info:
