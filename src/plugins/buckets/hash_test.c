...
int main(int argc, const char *argv[]) {
	...
	ulog(LOG_INFO, "There are %zu empty buckets (%u%%) and the maximal size is %hhu\n", zero_count, (unsigned) (zero_count * 100LLU / 0x100000000LLU), max);
	for (uint64_t i = 0; i < 0x100000000LLU; i ++)
		if (counters[i] == max)
			ulog(LOG_INFO, "Max at position %u\n", (unsigned) i);
	ulog(LOG_INFO, "Histogram:\n");
