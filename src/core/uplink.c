...
static bool uplink_connect_internal(struct uplink *uplink) {
	...
	if (socat) {
		...
		close(sockets[1]);
		const char *remote = mem_pool_printf(loop_temp_pool(uplink->loop), "OPENSSL:%s:%s,cafile=/etc/certs/ucollect.pem,compress=auto", uplink->remote_name, uplink->service);
